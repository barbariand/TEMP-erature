...
#define LCD_CMD_SLPIN \
  (0x10)  // Go into sleep mode (DC/DC, oscillator, scanning stopped, but memory keeps content)
#endif
...
LilyGo_AMOLED::LilyGo_AMOLED()
    : boards(NULL), _hasRTC(false), _disableTouch(false) {
  spiDev = NULL;
  pBuffer = NULL;
  spi = NULL;
...
uint16_t LilyGo_AMOLED::getBattVoltage(void) {
  if (boards) {
    if (boards->pmu) {
      ...
      v1 *=
          2;  //The hardware voltage divider resistor is half of the actual voltage, multiply it by 2 to get the true voltage
#endif
      return v1;
    }
  }
...
bool LilyGo_AMOLED::initPMU() {
  bool res = XPowersAXP2101::init(Wire, boards->pmu->sda, boards->pmu->scl,
...
bool LilyGo_AMOLED::initBUS(DriverBusType type) {
  ...
  if (type == QSPI_DRIVER) {
    ...
    esp_err_t ret =
        spi_bus_initialize(DEFAULT_SPI_HANDLER, &buscfg, SPI_DMA_CH_AUTO);
    if (ret != ESP_OK) {
  ...
  if (touchFunc && boards->touch) {
    if (boards->touch->sda != -1 && boards->touch->scl != -1) {
      ...
      if (Wire.endTransmission() == 0) {
        ...
        bool res = TouchDrvCSTXXX::begin(
            Wire, CST816_SLAVE_ADDRESS, boards->touch->sda, boards->touch->scl);
        if (!res) {
          ...
          _touchOnline = true;
          TouchDrvCSTXXX::setCenterButtonCoordinate(600,
                                                    120);  //AMOLED 1.91 inch
        }
      }
    }
  } else {
...
bool LilyGo_AMOLED::beginAMOLED_241(bool disable_sd, bool disable_state_led) {
  ...
  if (boards->touch) {
    ...
    if (Wire.endTransmission() == 0) {
      ...
      TouchDrvCSTXXX::setPins(boards->touch->rst, boards->touch->irq);
      bool res = TouchDrvCSTXXX::begin(Wire, CST226SE_SLAVE_ADDRESS,
                                       boards->touch->sda, boards->touch->scl);
      if (!res) {
...
bool LilyGo_AMOLED::beginAMOLED_147() {
  ...
  _touchOnline = TouchDrvCHSC5816::begin(
      Wire, CHSC5816_SLAVE_ADDRESS, boards->touch->sda, boards->touch->scl);
  if (!_touchOnline) {
  ...
  // Share I2C Bus
  bool res = SensorCM32181::begin(Wire, CM32181_SLAVE_ADDRESS,
                                  boards->sensor->sda, boards->sensor->scl);
  if (!res) {
    ...
    /*
            Sensitivity mode selection
                ...
                SAMPLING_X1_4
        */
    SensorCM32181::setSampling(SensorCM32181::SAMPLING_X2), powerOn();
  }
...
void LilyGo_AMOLED::writeCommand(uint32_t cmd, uint8_t* pdat, uint32_t length) {
  if (spiDev) {
    ...
    spiDev->beginTransaction(
        SPISettings(boards->display.freq, MSBFIRST, TFT_SPI_MODE));
...
void LilyGo_AMOLED::setAddrWindow(uint16_t xs, uint16_t ys, uint16_t xe,
                                  uint16_t ye) {
  xs += _offset_x;
  ys += _offset_y;
  xe += _offset_x;
...
void LilyGo_AMOLED::pushColors(uint16_t x, uint16_t y, uint16_t width,
...
void LilyGo_AMOLED::setRotation(uint8_t rotation) {
  ...
  if (boards == &BOARD_AMOLED_191 || boards == &BOARD_AMOLED_191_SPI) {
    ...
    log_e(
        "The screen you are currently using does not support screen "
        "rotation!!!");
  }
}
